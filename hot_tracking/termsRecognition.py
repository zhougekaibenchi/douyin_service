...
class TermsRecognition(object):
	...
	def get_idf(self,jieba_content):
		...
		try:
			...
			diction = pd.DataFrame( list(set(sum(jieba_content,())) ) , columns = ['words'])
		# diction['wtf'] = list(map(lambda x : self.get_word_freq(x,jieba_content)  , diction['words'] ))
	...
	def wordsGenerator(self,base_text , mode = 1):
		...
		if mode == 0 :
			for i in  tqdm(range(1,len(base_text)-2)):
	...
	def generate_word(self, sortKey):
		...
		self.word_get_dof()
		self.get_score()
		result = sorted(self.result.items(), key=lambda d:d[1][sortKey], reverse=True)
		if self.topK != -1:
...
if __name__ == '__main__':
	douyin_data_file = '../result/douyin_dataset.xls'
	...
	# ----------- 未分词 -----------
	generator = TermsRecognition(content = data['title'], is_jieba=False, topK=50 , mode=[1])   # 文字版
	# 全部发现
	result_dict = generator.generate_word(sortKey="score")
    ...
    #
    #
    # generator = termsRecognition(content = keywords[:100] , is_jieba=True , topK = 20,mode = [1]) #图像版
